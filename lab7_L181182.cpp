...
void printAccounts(bankAccount **arr1,int size);
bool makeTransaction(bankAccount *ptrarr,int size,int id,float amount,string transtype);
//void deleteAccounts(....);
...

{
	...
	*/
	...
	bankAccount **ar=new bankAccount*[2];
	float x[2]={-5000,5000};
	ar[0]=new bankAccount(50,2000,x,2);
	ar[1]=new bankAccount(60,6000,400000);
	ar[0]->withdraw(1000);
	for(int i=0;i<2;i++)
		ar[i]->print();
...
bool makeTransaction(bankAccount *ptrar,int size,int id,float amount,string transtype)
{
	for(int i=0;i<size;i++)
	{
		if(ptrar->getID()==id)
		{
			if(ptrar->getbal<ptrar->getballim)
			{
				ptrar->deposit(amount);
				return true;
			}
			else 
				return false;
			if(amount<ptrar->getbal)
			{
				ptrar->withdraw(amount);
		          return true;
			}
			else 
				return false;
